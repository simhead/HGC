...
def deploy(envName, buildNumber) {
	...
	for (i = 0; i <jsonObject.name.size(); i++) {
		...
		if(jsonObject.name[i]==apimConf['organizationId']){
		  ...
		  prJsonData.version = buildNumber as String
		  ...
		  curlStr = "curl --insecure -X GET --user "+apimConf['axwayUser']+":"+apimConf['axwayPasswd']+" \"https://"+svrName+":"+apimConf['PORT']+"/api/portal/v1.3/proxies?field=name&op=like&value=petstore_"+buildNumber+"\""
		  ...
		  if (jsonObject.name.size() == 1) {
			...
			if (jsonObject.state[0] instanceof String && jsonObject.state[0] == "unpublished") {
				...
				echo "Enable Frontend service to PUBLISHED"
				curlStr = "curl --insecure -X POST --user "+apimConf['axwayUser']+":"+apimConf['axwayPasswd']+ " -d \"name="+jsonObject.name[0]+"&vhost="+svrName+"\" \"https://"+svrName+":"+apimConf['PORT']+"/api/portal/v1.3/proxies/"+jsonObject.id[0]+"/publish\""			  
				echo curlStr;
				sh "${curlStr}"
			}
		  } else
